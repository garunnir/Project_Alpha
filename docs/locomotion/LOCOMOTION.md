...
- 공중 여부는 논리 바닥 수직 해석의 잔여 수직 속도를 사용한다. 공중 수평 조작 가속도는 기본 3 m/s²이며 무입력은 수평 속도를 보존한다. 0.06초 이상 공중이면 `Authored/Falling Loop.anim`을 반복 재생한다. 원본 Falling Idle의 복사본만 loop로 설정했다. 낙하 중 지상 출발/정지/반전 모션과 발 IK는 중단한다. 중력과 논리 바닥 충돌은 기존 경로를 유지한다.
- 착지는 물리 틱의 순번·충돌 직전 수직 속도·공중 시간·수평 속도를 보존해 애니메이션 틱에서 한 번 소비한다. 공중 시간이 0.12초 미만이면 착지 동작을 생략한다. 충격 속도 7 m/s 미만은 일반 착지, 그 이상은 강한 착지다. 강한 착지 중 수평 속도가 4.5 m/s 이상이고 입력이 진행 방향 60도 이내이면 구르기를 선택한다. 수영·잠수·담넘기·스크립트 이동·Hurt·Work 동안 발생한 착지는 재생하지 않고 소비한다.
- 일반 착지는 이동 입력이 있으면 0.25초 뒤 보행으로 블렌딩한다. 강한 착지는 재생 구간의 45–85%에서 이동을 회복한다. 구르기는 outgoing fade 전까지 착지 당시 방향을 유지하고 수평 속도를 2 m/s²로 줄인다. 재낙하 또는 상위 행동으로 전환하면 방향 고정을 해제한다. 구르기 중 발 IK는 제외한다.

| 착지 | 원본 구간(초) | 배속 |
|---|---|---|
| 일반 | Falling To Landing, 8/30–29/30 | 1.2 |
| 강한 충격 | Hard Landing, 4/30–52/30 | 1.35 |
| 전진 구르기 | Falling To Roll, 15/30–55/30 | 1.15 |

Blender 원본의 발 높이를 확인해 접촉 이후 구간부터 재생한다. 별도 점프 입력/도약 물리는 이번 작업에 포함하지 않으므로 Jumping Up/Running Jump와 추락 피해용 Falling Flat Impact는 연결하지 않는다. `LocomotionTransitionChecks.CheckAir()`는 낙하 반복·착지 선택·일시정지·중단·재낙하·작은 단차·구르기 관성을 검증한다. `BeginDrop(height)`/`DropReport()`는 폐기할 Play 세션에서 실제 물리 낙하와 애니메이션 관찰에 사용한다.
